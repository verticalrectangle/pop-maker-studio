#version 300 es
// Skin smooth — edge-preserving bilateral blur masked to skin tones (YCbCr).
precision highp float;
uniform sampler2D u_tex;
uniform vec2 u_res;
uniform float u_amount;   // 0..1 strength
uniform float u_p0;       // radius in px (default 4)
uniform float u_p1;       // tone permissiveness 0..1 (default 0.5)
in vec2 v_uv;
out vec4 frag;
