...
// The offer listener must cover every wl_data_offer event: libwayland aborts
// the process when a compositor (e.g. niri) delivers an event whose listener
// slot is NULL. GLFW only uses `offer` (text/uri-list probing); the
// source_actions/action events from a drag source that sets actions (file
// managers do) are accepted silently. See glfw/glfw#2835 / #2886.
