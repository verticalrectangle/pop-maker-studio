// app_state.cpp — core state machinery hoisted from app.cpp during the iOS
// engine extraction (Phase 0): keyframe tracks (PropTrack), the keyframe
// field table, Clip::eval_prop, clip split/shift/flash, FX brick coupling +
// glass resolution, the effect/creative-FX collectors, and beat_pulse_at.
// app.cpp keeps app_init/app_frame (the ImGui root window). Behavior
// unchanged.
#include "app.h"
#include "history.h"
#include "paths.h"
...
static void accum_creative_clip(CreativeFXAccum& acc, const Clip& cl, float _cl_beat_pulse, float _cl_t = 0.f) {
    ...
    switch (cl.fx_type) {
        ...
        case FXType::ChromaMelt:
            ...
            // slot for a trippy temporal smear. Params keyframable via eval_prop.
            acc.chroma_melt_on        = true; acc.any_cfx = true;
            acc.chroma_melt_r         = cl.eval_prop("fx_chroma_melt_r", _cl_t);
            acc.chroma_melt_g         = cl.eval_prop("fx_chroma_melt_g", _cl_t);
