...
static std::mutex         g_samples_mtx;  // guards swap at end of load
...
bool audio_load(const std::string& path) {
    ...
    audio_shutdown();
    {
        std::lock_guard<std::mutex> lk(g_samples_mtx);
        g_samples.clear();
        g_read_pos = 0;
        g_duration = 0.f;
    }
    ...
    std::thread([path]() {
        ...
        if (audio_stream < 0) { avformat_close_input(&fmt_ctx); g_loading.store(false); return; }
        ...
        while (av_read_frame(fmt_ctx, pkt) >= 0) {
            ...
            while (avcodec_receive_frame(codec_ctx, frm) == 0) {
                ...
                std::vector<float> chunk(out_samples * 2);
                uint8_t* out_ptr = (uint8_t*)chunk.data();
                int converted = swr_convert(swr, &out_ptr, out_samples,
                    (const uint8_t**)frm->data, frm->nb_samples);
                if (converted > 0)
                    tmp.insert(tmp.end(), chunk.begin(), chunk.begin() + converted * 2);
                av_frame_unref(frm);
            }
            av_packet_unref(pkt);
        }

        float dur = (float)tmp.size() / 2.f / (float)sample_rate;
        ...
        // Swap in decoded samples — device is not running yet so no callback race
        {
            std::lock_guard<std::mutex> lk(g_samples_mtx);
